...
Locate `notion.json` relative to this SKILL.md:
  Mac:     `~/.claude/plugins/beround-security/config/notion.json`
  Windows: `C:\Users\<USERNAME>\.claude\plugins\beround-security\config\notion.json`

Parse it with:
```python
import json, pathlib, os
if os.name == 'nt':
    config_path = pathlib.Path(os.environ["USERPROFILE"]) / ".claude" / "plugins" / "beround-security" / "config" / "notion.json"
else:
    config_path = pathlib.Path.home() / ".claude" / "plugins" / "beround-security" / "config" / "notion.json"
...
Mac/Linux:
  ...
  HOOKS_DIR = os.path.expanduser("~/.claude/plugins/beround-security/hooks")
...
Windows (Git Bash / PowerShell):
  ...
  HOOKS_DIR = os.path.join(os.environ["USERPROFILE"], ".claude", "plugins", "beround-security", "hooks")
